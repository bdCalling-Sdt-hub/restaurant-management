...
class _BookNowState extends State<BookingDetailsScreen> {
  ...
  Widget build(BuildContext context) {
    return GetBuilder<GetSingleBookingController>(
      builder: (controller) {
        return Scaffold(
          appBar: AppBar(
            toolbarHeight: 80,
            ...
            actions: [
             controller.model.data?[0].status !="Booked"? Container(
               margin: EdgeInsets.only(right: 8),
               decoration:BoxDecoration(color: AppColors.greenNormal,borderRadius: BorderRadius.circular(8)),
               child: TextButton(onPressed: (){
                 Get.toNamed(AppRoute.reviewScreen,arguments: controller.model.data?[0].restaurant);
                 print("========================ID${controller.model.data?[0].restaurant}");
               }, child: const CustomText(text: "Review",color: AppColors.whiteColor,fontWeight: FontWeight.w500,)))
